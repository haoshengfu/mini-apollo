/******************************************************************************
BlockerManager类用于管理Blocker，其中维护了以channel为键值的Blocker的map。Reader::Enqueue()函数将消息放到Blocker的成员published_msg_queue_队列中。之后，可以通过Blocker::Observe()函数将成员published_msg_queue_队列的消息放到成员observed_msg_queue_队列，然后通过Blocker::GetLatestObserved()函数得到最新的消息。
 *****************************************************************************/
...
class BlockerManager {
 ...
 private:
  ...
  BlockerMap blockers_;  //管理blocker
